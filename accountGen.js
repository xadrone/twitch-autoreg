...
async function get_username() {
  ...
  return userValidJson.data.isUsernameAvailable
    ? userJson[0]
    : userJson[0] + Math.floor(Math.random() * 888);
}

async function accountGen() {
  //exitCode: (0) - ошибок нет, (-1) - ошибка подключения mongoDB, (1) - ошибка создания twitch
  const browser = await chromium.launchPersistentContext(
    'C:/Users/hyrag/AppData/Local/Google/Chrome/User Data/',
    {
      headless: false,
      channel: 'chrome',
      slowMo: 200,
  ...
  await page
    ...
    .filter({ hasText: 'Skip to...AltPBrowseBrowseBrowseSearchLog InSign Up' })
    .getByRole('button', { name: 'Sign Up' })
    .click();
  await page.getByLabel('Username').fill(username);
  await page.getByRole('textbox', { name: 'Create a secure password' }).click();
  await page
    .getByRole('textbox', { name: 'Create a secure password' })
    .fill(password);
  ...
  await page.getByRole('button', { name: 'Next Step' }).click();
  await page
    .getByRole('combobox', { name: 'Select your birthday month' })
    .selectOption(Math.floor(Math.random() * 11 + 1).toString());
  await page.getByPlaceholder('Day').click();
  await page
    .getByPlaceholder('Day')
    .fill(Math.floor(Math.random() * 28 + 1).toString());
  await page.getByPlaceholder('Year').click();
  await page
    .getByPlaceholder('Year')
    .fill(Math.floor(Math.random() * 30 + 1980).toString());
  ...
  await page
    ...
    .then(
      async () => {
        ...
        await browser.close();
        try {
          await client.connect().then(async (mongoClient) => {
            await client
              .db('accountsDB')
              .collection('accounts')
              .insertOne({ login: username, pass: password });
